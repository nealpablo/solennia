...
document.addEventListener('click', (e) => { if (!notifPanel || !notifBtn) return; if (!notifPanel.contains(e.target) && !notifBtn.contains(e.target)) toggleNotif(false); });
...
function collectSearchables() {
  ...
  document.querySelectorAll('a[href]').forEach(a => {
    ...
    const key = label.toLowerCase() + '|' + href; if (seen.has(key)) return; seen.add(key);
...
searchInput?.addEventListener('keydown', (e) => {
  if (e.key === 'Enter') {
    ...
    if (q) { localStorage.setItem('solennia_last_search', q); window.location.href = `${base}?q=${encodeURIComponent(q)}`; }
...
function toggleMenu(show) {
  ...
  if (typeof show === 'boolean') profileMenu.classList.toggle('hidden', !show);
...
document.addEventListener('click', (e) => {
  ...
  if (!profileMenu.contains(e.target) && !profileBtn.contains(e.target)) {
    profileMenu.classList.add('hidden'); profileBtn.setAttribute('aria-expanded', 'false');
  }
});
...
function closeModals() { backdrop?.classList.add('hidden'); loginModal?.classList.add('hidden'); registerModal?.classList.add('hidden'); }
...
async function refreshProfileFromServerIfNeeded() {
  ...
  try {
    ...
    if (res.ok && json?.success && json.user) {
      const { first_name = '', last_name = '', username = '', email = '', role = 0 } = json.user;
      localStorage.setItem('solennia_profile', JSON.stringify({ first_name, last_name, username, email }));
      if (typeof role === 'number') localStorage.setItem('solennia_role', String(role));
    }
  } catch (_) { /* ignore */ }
  finally {
...
loginForm?.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    localStorage.setItem('solennia_role', String(res.role ?? 0));
    await refreshProfileFromServerIfNeeded();
    err?.classList.add('hidden'); closeModals(); setAuthUI();
    alert('Welcome back!');
  } catch (ex) {
...
menuLogout?.addEventListener('click', () => {
  ...
  localStorage.removeItem('solennia_role');
  setAuthUI(); applyProfileNameFromCache(); toggleMenu(false);
  alert('Signed out.');
});
...
function openVendorFlow() {
  const token = localStorage.getItem('solennia_token');
  toggleMenu(false);
  if (!token) { alert('Please log in or register to become a vendor.'); return; }
  vendorTerms?.classList.remove('hidden');
}
...
vendorClose.forEach(btn => btn.addEventListener('click', () => {
  vendorTerms?.classList.add('hidden'); vendorBackground?.classList.add('hidden'); vendorMedia?.classList.add('hidden');
}));
agreeTerms?.addEventListener('click', () => { vendorTerms?.classList.add('hidden'); vendorBackground?.classList.remove('hidden'); });
toMedia?.addEventListener('click', () => { vendorBackground?.classList.add('hidden'); vendorMedia?.classList.remove('hidden'); });
...
submitVendor?.addEventListener('click', (e) => {
  ...
  fetch(`${API}/vendor/apply`, { method: 'POST', headers: authHeaders(), body: JSON.stringify(payload) })
    .then(async (res) => {
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json?.error || json?.message || 'Failed');
      alert('Vendor application submitted!'); vendorMedia?.classList.add('hidden');
    })
...
feedbackLink?.addEventListener('click', (e) => {
  e.preventDefault();
  const token = localStorage.getItem('solennia_token');
  if (!token) { alert('You need to log in before giving feedback.'); return; }
...
feedbackForm?.addEventListener('submit', async (e) => {
  ...
  try {
    const res = await fetch(`${API}/feedback`, { method: 'POST', headers: authHeaders(), body: JSON.stringify(formData) });
    const json = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(json?.error || 'Failed to send feedback');
    alert('Thank you for your feedback!'); feedbackModal?.classList.add('hidden'); feedbackForm.reset();
  } catch (err) { alert(err.message || 'Error sending feedback'); }
});
...
(() => {
  ...
  const pxPerMs = 0.082;
  let loopWidth = 0; let last = performance.now(); let pausedUntil = 0;
  container.style.overflowX = 'auto';
  function computeLoopWidth() { loopWidth = track.scrollWidth / 2; }
  computeLoopWidth(); window.addEventListener('resize', computeLoopWidth);
  function loop(now) {
...
/* 🔥 Apply cached name then refresh from server */
applyProfileNameFromCache?.();
refreshProfileFromServerIfNeeded?.();

/* 🔧 FIX: remove crashing code that referenced undefined "json"
   Instead, if role in localStorage is admin (2), ensure Admin link visible. */
try {
  const role = parseInt(localStorage.getItem('solennia_role') || '0', 10);
  const menuProfile = document.getElementById('menuProfile');
  const menuAdmin = document.getElementById('menuAdmin');
  if (role === 2 && menuProfile && !menuAdmin) {
